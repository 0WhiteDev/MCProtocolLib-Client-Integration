...
import us.whitedev.proxy.listeners.ClientListener;
...
public class SessionAction {
    private static SessionAction sessionAction;
    ...
    public void createNewSession(String name, String host, int port, Proxy proxy) {
        ...
        client.getSession().addListener(new ClientListener(name));
    ...
    public void createNewEasyMCSession(String token, String host, int port, Proxy proxy) {
        ...
        Client client = new Client(host, port, protocol, getTcpSessionFactory(proxy));
        client.getSession().connect();
        client.getSession().addListener(new ClientListener(token));
    ...
    public void disconnectSession(String[] names) {
        for (String name : names) {
            if (name != null) {
                Client client = SessionsRep.getInstance().getOnlineUsers().get(name);
                SessionsRep.getInstance().removeOnlineUser(name);
                client.getSession().disconnect("closed");
            }
        }
    }
}
